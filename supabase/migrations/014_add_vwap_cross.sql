-- Add VWAP Cross indicator

INSERT INTO indicators (name, config, is_active)
VALUES
  ('vwap_cross', '{"use_close_confirmation": 1}', false)
ON CONFLICT (name) DO NOTHING;

-- Ensure account row exists for VWAP Cross
...
SELECT
  ...
  false
FROM indicators i
WHERE i.name = 'vwap_cross'
ON CONFLICT (indicator_id) DO NOTHING;

-- Ensure performance metrics row exists for VWAP Cross
